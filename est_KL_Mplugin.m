function est = est_KL_Mplugin(samp_p,samp_q)
% est = est_KL_Mplugin(samp_p,samp_q)

% Modified plug-in estimation of KL divergence (in nats) of the input samples
%
...
% Error-check
    ...
    for i=1:wid_q
        ...
        est(i) = sum((hist_p/m).*log((hist_p/m)./((hist_q+(hist_q==0))/n)));
    end
